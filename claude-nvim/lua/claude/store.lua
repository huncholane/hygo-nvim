...
function M.session_path(id, cwd)
  return cwd_dir(cwd) .. "/session-" .. id .. ".json"
end

local function pointer_path(cwd)
  return cwd_dir(cwd) .. "/last.json"
end

function M.write_pointer(info)
  local f = io.open(pointer_path(info and info.cwd), "w")
...
function M.read_pointer(cwd)
  local f = io.open(pointer_path(cwd), "r")
...
function M.load(id, cwd)
  return M.load_path(M.session_path(id, cwd))
end

function M.save(session)
  local f = io.open(M.session_path(session.id, session.cwd), "w")
  if not f then return false end
...
function M.all_prompts(cwd)
  local out = {}
  for _, entry in ipairs(M.list_sessions(cwd)) do
